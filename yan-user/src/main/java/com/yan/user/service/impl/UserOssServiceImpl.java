...
@Service
public class UserOssServiceImpl implements IUserOssService, OssService {
    ...
    public List<SysOssVo> listByIds(Collection<Long> ossIds) {
        ...
        UserOssServiceImpl ossService = SpringUtils.getAopProxy(this);
        for (Long id : ossIds) {
     ...
     */
    @Override
    public Path fileDownload(String fileService, String path) {
        OssClient storage = OssFactory.instance(fileService);
        return storage.fileDownload(path);
    }
     ...
     */
    @Override
    public SysOssVo upload(InputStream inputStream,String fileName, Long fileSize,String contentType) {
        ...
        // 保存文件信息
        return buildResultEntity(fileName, suffix, storage.getConfigKey(), uploadResult, 0L);
    }

    @Override
    public SysOssVo uploadWithOriginalName(InputStream inputStream,String fileName, Long fileSize,String contentType) {
        String suffix = FileUtil.getSuffix(fileName);
        OssClient storage = OssFactory.instance();

        UploadResult uploadResult = storage.uploadWithOriginalName(inputStream, fileName, fileSize, contentType);
        // 保存文件信息
        return buildResultEntity(fileName, suffix, storage.getConfigKey(), uploadResult, 0L);
    }
    ...
    private SysOssVo buildResultEntity(String originalfileName, String suffix, String configKey, UploadResult uploadResult) {
        return buildResultEntity(originalfileName, suffix, configKey, uploadResult, 0L);
    }

    @NotNull
    private SysOssVo buildResultEntity(String originalfileName, String suffix, String configKey, UploadResult uploadResult, Long createBy) {
