...
public class WithdrawService {
	...
	public void withdraw(MerchantRechargeWithdraw withdraw) {
		...
		withdraw.setPayAddress(credentials.getAddress());
		...
		if(assetTypeCache.isNativeToken(withdraw.getChain(), withdraw.getSymbol())){
			...
			if (amount.compareTo(ethBalance.subtract(fee).subtract(txFee)) > 0) {
				log.error("[{}] 热钱包余额不足 {} {} - 需要: {}, 当前: {}", withdraw.getChain(), assetType.getSymbol(), generalHotAddress.getHotAddress(), amount.add(txFee).add(fee), ethBalance);
				return;
			}
			BigInteger nonce = nonceManager.getNextNonce(assetType.getChain(),  credentials.getAddress());
			try {
				txId = ethService.sendEth(assetType.getChain(), credentials, withdraw.getReceiveAddress(), amount, nonce);
			...
			if (amount.compareTo(erc20Balance.subtract(fee)) > 0) {
				log.error("[{}] 热钱包余额不足 {} {} - 需要: {}, 当前: {}", withdraw.getChain(), assetType.getSymbol(), generalHotAddress.getHotAddress(), amount.add(fee), erc20Balance);
				return ;
			}

			BigInteger nonce = nonceManager.getNextNonce(assetType.getChain(),  credentials.getAddress());
			try {
				txId = ethService.sendErc20(assetType.getChain(), credentials, assetType.getContractAddress(), withdraw.getReceiveAddress(), amount, nonce);
			}catch (RuntimeException e) {
				nonceManager.confirmNonce(assetType.getChain(), credentials.getAddress(),  nonce);
				throw new RuntimeException(e);
			}
